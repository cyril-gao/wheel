...
public class WildcardMatching {
    /*
    private boolean impl1(String s, String p) {
        boolean retval = s == p;
        if (!retval) {
            if (s != null && p != null) {
                int sl = s.length();
                ...
                while (pi < pl && retval) {
                    ...
                    if (pc != '*') {
                        ...
                        int i;
                        for (i = pi + 1; i < pl && p.charAt(i) == '*'; ++i) {}
                        pi = i - 1;
                        retval = false;
                        String subp = p.substring(pi+1);
                        for (i = sl; i >= si; --i) {
    ...
    */
                        ...
                        while (true) {
                            if (subpPosition < 0) {
                                ...
                                while (true) {
                                    ...
                                    if (
                                        impl2(s.substring(0, subpPosition), 0, p.substring(0, pi), 0, f1) &&
                                        impl2(s.substring(subpPosition + subpLen), 0, p.substring(pi + subpLen), 0, f2)
                                    ) {
                                        return true;
                                    } else {
                                        if (f1[0] || f2[0]) {
    ...
    private boolean impl2(String s, int sStart, String p, int pStart, boolean[] failedImmediately) {
        ...
        //System.out.printf("txt:     %s\npattern: %s\n\n", s.substring(sStart), p.substring(pStart));
        int sl = s.length();
        int pl = p.length();
